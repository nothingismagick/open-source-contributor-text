...
本介紹不可能解釋所有不同代碼授權之間的細微差異，因為有多達數十個授權存在。 我們可以於可分配時在授權文件標題中、在存儲庫根層的授權文件中、在項目的README檔、或可能在package.json (如使用node) 放入引用來源來將授權加入到代碼中。

如果您跳過了上文，沒有閱讀前面的章節，以下是三種主要授權類型的快速回顧和每種類型的常見例子，以及進入更多詳細資訊和分析的兩個連結。
...
> 在Quasar框架中，所有對核心庫的貢獻都必須是遵循MIT授權協議的。 在參與任何貢獻時，已代表你同意此授權。

### 文本/文檔的重要授權

文本和文檔也是一項創造性的作品，一般情況下也受版權法的保護。 您可以通過選擇共享創意 (Creative Commons) 資源或自由文檔授權 (Free Document License)，將這些權利轉讓給第三方或你將向其做出貢獻的項目擁有者。

在授權文檔結尾命名授權和作者往往是最好做法。

- [共享創意](https://creativecommons.org/)
- [自由文檔授權](https://www.gnu.org/licenses/fdl.html)

> 在Quasar框架中，所有文檔都是 (或將) 根據 FDL (自由文檔授權) 授權的。

### 授權藝術品/設計/影片

對於這類貢獻，當中的法律規定就像地球上的國家一樣多，所以詳細瞭解各國之間的不同是絕不合適的。 無論如何，創作圖像的那位絕不會失去說自己是擁有者的權利 (除非有一份絕對嚴厲的合約阻止了這一點 - 在某些情況下，這在法庭上是站不住腳的)。 這就是為什麼會有一條通用規則：引用作者並命名授權。 如果你做不到這一點，那就不要使用該資產。
...
在 [Google Fonts (谷歌字體) ](https://fonts.google.com/attribution) 列出的數百種字體中，準確有兩個不同的授權被使用：Apache 授權 v2 ，或 SIL 開源字體授權 v1.1。它們絕大多數是OFL (Open Font License 開源字體授權)，基本上是與MIT相同類型的寬鬆授權。

如果您使用一種字體，就需要確保你確實擁有該字體的授權，因為某些專業字體創造將根據其應用來銷售不同類型的授權。 (如桌面發佈的一個授權、網站發佈的另一個授權等) 如果你使用的是開源字體，最佳做法是將授權的副本放在字體的資料夾中。

此外，一些如FontSquirrel的服務提供了一個 [網上字體產生器](https://www.fontsquirrel.com/tools/webfont-generator)，使設計人員能夠製作一組字體文件，以便各個瀏覽器可以選擇最適合的類型。 它們要求你驗證自己是否有權將字體轉換為其他格式。 SIL及OFL確實允許這樣做。 如果你找不到授權，請假設你沒有使用該字體的權利。

- [OFL - 常見問題網頁版本 (1.1-update5)](https://scripts.sil.org/cms/scripts/page.php?item_id=OFL-FAQ_web)

# 結語

僅僅因為你 "開放" 了你的作品讓別人協作，還不是最後。 僅僅因為你在 Photoshop 文件上拍上了共享創意授權，並不意味著它是開放和免費的。 資產或項目的開放程度也由作業系統以及使用和修改文件所需的軟體決定。 請考慮不僅公佈你的工作過程的結果，而且告訴人們你是如何做到的。

此外，在從事開源項目時，請不要使用盜版軟體。 這不僅是 "不酷"，損害了整個開源社區的榮譽，而且讓你自己和你的同事處於危險之中，特別是如果你有高價值的憑據，如伺服器登錄等。 沒有比使用盗版更好的方法來讓駭客攻擊自己了 (可能還有你的團隊)。
